      ...
      <nav ref="mainMenu" v-if="mobileMenuIsActive" class="flex flex-col items-end">
        <ul
          ...
          class="block"
            ...
            <svg @click="expand = !expand" v-if="item?.childItems?.nodes?.length" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="ml-2 w-6 h-6 inline-block">
              <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
            </svg>
            <a class="block mr-8 text-lg " v-if="expand" v-for="child in item?.childItems?.nodes" :href="item?.connectedNode?.node?.uri + '#'">{{child?.label}}</a>
          </li>
        </ul>
      </nav>
    </div>
